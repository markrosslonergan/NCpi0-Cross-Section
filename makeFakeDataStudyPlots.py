  ...
  for sigDefexcl in ["exclusive","inclusive"]:
    ...
    with makeEnv_TCanvas('{0}/fakedatavsgenie_evtRate_{1}.png'.format(plotDir,sigDef)) as canvas:
      ...
      # If configured to test mode, also draw and print chi2 of the unsmeared distribution for non-closure tests.
        ...
        pt.SetBorderSize(0)
        pt.SetFillColorAlpha(0, 0)
        pt.AddText("#chi^{{2}}_{{unsmeared}}/DOF: {0:.0f}/{1}".format(chi2_GENIE, upperBound + include_underflow_true))
        pt.AddText("#chi^{{2}}_{{smeared}}/DOF: {0:.2f}/{1}".format(chi2_GENIE_smeared, upperBound + include_underflow_true))
        pt.Draw()
        canvas.canvas.cd(0)
    ...
    with makeEnv_TCanvas('{0}/fakedatavsgenie_xSection_{1}.png'.format(plotDir,sigDef)) as canvas:
      ...
      if is_test:
        ...
        pt = ROOT.TPaveText(0.53, 0.6, 0.775, 0.7, "NDC")
        pt.AddText("#chi^{{2}}/DOF: {0:.2f}/{1}".format(chi2_GENIE_smeared, upperBound + include_underflow_true))
        ptall.Draw()
      pt.SetBorderSize(0)
      pt.SetFillColorAlpha(0, 0)
      pt.Draw()
    ...
    with makeEnv_TCanvas('{0}/nuwro_evtRate_{1}.png'.format(plotDir,sigDef)) as canvas:
      ...
      if is_test:
        ...
        pt.SetBorderSize(0)
        pt.SetFillColorAlpha(0, 0)
        pt.AddText("#chi^{{2}}_{{unsmeared}}/DOF: {0:.0f}/{1}".format(chi2_NuWro, upperBound + include_underflow_true))
        pt.AddText("#chi^{{2}}_{{smeared}}/DOF: {0:.2f}/{1}".format(chi2_NuWro_smeared, upperBound + include_underflow_true))
        pt.Draw()
      else:
        pt = ROOT.TPaveText(0.53, 0.6, 0.775, 0.7, "NDC")
        pt.SetBorderSize(0)
        pt.SetFillColorAlpha(0, 0)
        pt.AddText("#chi^{{2}}/DOF: {0:.2f}/{1}".format(chi2_NuWro_smeared, upperBound + include_underflow_true))
        pt.Draw()
        ptall.Draw()
      canvas.canvas.cd(0)
    ...
    with makeEnv_TCanvas('{0}/fakedatavsgenie_evtRate_folded_{1}.png'.format(plotDir,sigDef)) as canvas:
      ...
      if is_closure_test:
        ...
        pt.SetBorderSize(0)
        pt.SetFillColorAlpha(0, 0)
        pt.AddText("#chi^{{2}}_{{ResponseMaker}}/DOF: {0:.2f}/{1}".format(chi2_effNum_hreco, upperBound + include_underflow_reco))
        pt.AddText("#chi^{{2}}_{{Folded}}/DOF: {0:.2f}/{1}".format(chi2_effNum_genie_evtRate, upperBound + include_underflow_reco))
        pt.Draw()
      else:
        ...
        pt.SetBorderSize(0)
        pt.SetFillColorAlpha(0, 0)
        pt.AddText("#chi^{{2}}/DOF: {0:.2f}/{1}".format(chi2_evtRate_reco_effNum, upperBound + include_underflow_reco))
        pt.Draw()
      legend.Draw()
      ptall.Draw()
      canvas.canvas.cd(0)
    ...
    with makeEnv_TCanvas('{0}/nuwro_evtRate_folded_{1}.png'.format(plotDir, sigDef)) as canvas:
      ...
      pt.SetBorderSize(0)
      pt.SetFillColorAlpha(0, 0)
      pt.AddText("#chi^{{2}}/DOF: {0:.2f}/{1}".format(chi2_evtRate_reco_nuwro_signal, upperBound + include_underflow_reco))
      pt.Draw()
      ptall.Draw()
      canvas.canvas.cd(0)
